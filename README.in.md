...
One OpenAPI contract. {{count:backends}} backends that serve it — modern Java on the JDK's
own HTTP server, modern Python on the standard library, Spring with explicit configuration,
Spring Boot with auto-configuration. {{count:frontends}} frontends that consume it — Angular
...
not writing Java and writing Python, it is holding {{count:stacks}} implementations to one
...
```bash
make up          # all {{count:stacks}} stacks, plus Postgres
...
| | |
|---|---|
| Angular | http://localhost:{{port:angular}} |
| React | http://localhost:{{port:react}} |
| Side by side | http://localhost:{{port:side-by-side}} |
| Modern Java | http://localhost:{{port:java-modern}} |
| Traditional Spring | http://localhost:{{port:spring-traditional}} |
| Spring Boot | http://localhost:{{port:spring-boot}} |
| Modern Python | http://localhost:{{port:python-modern}} |
